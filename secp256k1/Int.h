...
static uint64_t inline _umul128(uint64_t a, uint64_t b, uint64_t *h) {
  ...
  *h = (uint64_t)(r >> 64);
  ...
  uint64_t hh = ah * bh;
  uint64_t mid = lh + hl;
  uint64_t carry = (mid < lh) ? 1ULL : 0ULL;
  *h = hh + (mid >> 32) + (carry << 32);
  return ll + (mid << 32);
#endif
}

static uint64_t inline __shiftright128(uint64_t hi, uint64_t lo, unsigned char n) {
  if (n == 0) return lo;
  if (n >= 64) return hi >> (n - 64);
  return (hi << (64 - n)) | (lo >> n);
}

static uint64_t inline __shiftleft128(uint64_t hi, uint64_t lo, unsigned char n) {
  if (n == 0) return hi;
  if (n >= 64) return lo << (n - 64);
  return (hi << n) | (lo >> (64 - n));
}

static unsigned char inline _subborrow_u64(unsigned char c, uint64_t a, uint64_t b, uint64_t *d) {
  // Portable correct implementation: detects borrow when a < b + c.
  // The previous __builtin_sub_overflow(a, b + c, d) was incorrect because
  // it ignored the intermediate carry generated by b + c.
  uint64_t diff = a - b - c;
  *d = diff;
...
static unsigned char inline _addcarry_u64(unsigned char c, uint64_t a, uint64_t b, uint64_t *d) {
  // Portable correct implementation: detects carry when a + b + c overflows.
  // The previous __builtin_add_overflow(a, b + c, d) was incorrect because
  // it ignored the intermediate carry generated by b + c.
  uint64_t sum = a + b + c;
  *d = sum;
