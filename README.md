...
### CPU Vectorization Backend (-A)

TrueCollider detects and selects the best SIMD instruction set available on your CPU:

`ash
# Auto-detect best SIMD (default)
./keyhunt -m address -f targets.txt -t 8

# Force a specific level
./keyhunt -m address -f targets.txt -A avx512 -t 8
./keyhunt -m address -f targets.txt -A avx2 -t 8
./keyhunt -m address -f targets.txt -A sse -t 8
./keyhunt -m address -f targets.txt -A none -t 8
`
...
A CUDA/OpenCL backend is available as scaffolding and can be selected at runtime:
...
GPU dispatch currently supports ddress, 
md160, and anity modes. Full EC math kernels are under construction; the dispatcher falls back to CPU when a mode is not yet GPU-ready.
...
| `-A mode` | CPU vectorization: none, sse, avx, avx2, avx512 (default: auto) |
| `-U backend` | GPU backend: none, cuda, opencl (experimental, default: none) |
| `-k value` | BSGS K factor (more RAM, faster) |
| `-z value` | Bloom filter size multiplier |
| `-G value` | CPU group/batch size (compile-time via `CPU_GRP_SIZE`) |
