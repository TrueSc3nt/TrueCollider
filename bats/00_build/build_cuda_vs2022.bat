...
if "!GEN_OK!"=="0" (
  ...
  if exist "!BDIR!\Release\keyhunt.exe" (
    copy /y "!BDIR!\Release\keyhunt.exe" keyhunt_cuda.exe
  ) else if exist "!BDIR!\keyhunt.exe" (
    copy /y "!BDIR!\keyhunt.exe" keyhunt_cuda.exe
  ) else (
    echo [E] keyhunt.exe missing after VS build
    exit /b 1
  )
  if errorlevel 1 (
    echo [E] Failed to copy keyhunt.exe to keyhunt_cuda.exe ^(is the exe locked?^)
    exit /b 1
  )
) else (
  echo [+] Building...
...
REM Package CUDA runtime DLL next to the exe (custom CUDA edition)
if defined CUDA_PATH (
  if exist "!CUDA_PATH!\bin\cudart64_12.dll" (
    copy /y "!CUDA_PATH!\bin\cudart64_12.dll" "%CD%\cudart64_12.dll" >nul
    echo [+] Packaged cudart64_12.dll
  )
  if exist "!CUDA_PATH!\bin\cudart64_13.dll" (
    copy /y "!CUDA_PATH!\bin\cudart64_13.dll" "%CD%\cudart64_13.dll" >nul
    echo [+] Packaged cudart64_13.dll
  )
)
...
echo PASS: CUDA build ^(custom edition^)
endlocal
exit /b 0
