...
REM Requires: keyhunt_cuda.exe (bats\00_build\build_cuda_vs2022.bat) + NVIDIA GPU + CUDA driver.
...
if not exist "%EXE%" (
  echo [E] %EXE% not found. Build with bats\00_build\build_cuda_vs2022.bat ^(or examples\build_cuda.bat^)
  exit /b 1
)
for %%F in ("%EXE%") do set "SZ=%%~zF"
if "!SZ!"=="0" (
  echo [E] %EXE% is 0 bytes ^(corrupt^). Rebuild with bats\00_build\build_cuda_vs2022.bat
  exit /b 1
)
