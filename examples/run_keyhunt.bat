...
if "!WANT_CUDA!"=="1" (
  if exist "!SCRIPT_DIR!keyhunt_cuda.exe" (
    ...
    echo [E] keyhunt_cuda.exe is 0 bytes ^(corrupt^). Rebuild with bats\00_build\build_cuda_vs2022.bat
    exit /b 1
  )
  echo [E] -U requires keyhunt_cuda.exe. Build with bats\00_build\build_cuda_vs2022.bat
  exit /b 1
)
...
if exist "!SCRIPT_DIR!keyhunt.exe" if "!KH_SIZE!"=="0" (
  echo [E] keyhunt.exe is 0 bytes ^(corrupt^). Rebuild with bats\00_build\build.bat / bats\00_build\build_mingw_native.bat
  exit /b 1
)
...
if errorlevel 1 (
  ...
  echo   bats\00_build\build.bat
  echo   REM or: bats\00_build\build_mingw_native.bat
  echo   REM CUDA: bats\00_build\build_cuda_vs2022.bat -^> keyhunt_cuda.exe
  echo.
  exit /b 1
)
