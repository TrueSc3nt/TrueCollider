...
static void hash160_batch_16x33(const uint8_t *inputs[16], uint8_t *outputs[16]) {
  if (g_backend_config.cpu_vector == CPU_VECTOR_AVX512) {
    hash160_avx512_16x33(inputs, outputs);
    return;
  }
  for (int i = 0; i < 16; i++) {
    uint8_t sha[32];
...
static void hash160_batch_16x65(const uint8_t *inputs[16], uint8_t *outputs[16]) {
  if (g_backend_config.cpu_vector == CPU_VECTOR_AVX512) {
    hash160_avx512_16x65(inputs, outputs);
    return;
  }
  for (int i = 0; i < 16; i++) {
    uint8_t sha[32];
...
static void hash160_batch_16x22(const uint8_t *inputs[16], uint8_t *outputs[16]) {
  if (g_backend_config.cpu_vector == CPU_VECTOR_AVX512) {
    hash160_avx512_16x22(inputs, outputs);
    return;
  }
  for (int i = 0; i < 16; i++) {
    uint8_t sha[32];
...
void Secp256K1::GetHash160_16(int type, bool compressed, Point pts[16], uint8_t *outputs[16]) {
#if HASH160_AVX512_AVAILABLE
  uint8_t keys[16][128];
  ...
  case BECH32:
    if (!compressed) {
      ...
      hash160_batch_16x65(in_ptrs, outputs);
    } else {
      for (int i = 0; i < 16; i++) {
      ...
      hash160_batch_16x33(in_ptrs, outputs);
    }
    break;
  ...
  {
    uint8_t p2pkh[16][20];
    uint8_t *p2pkh_ptrs[16];
    for (int i = 0; i < 16; i++) p2pkh_ptrs[i] = p2pkh[i];
    GetHash160_16(P2PKH, compressed, pts, p2pkh_ptrs);
    for (int i = 0; i < 16; i++) {
      keys[i][0] = 0x00;
      keys[i][1] = 0x14;
    ...
    hash160_batch_16x22(in_ptrs, outputs);
  }
  break;
...
void Secp256K1::GetHash160_fromX_16(int type, unsigned char prefix, Int *kx[16], uint8_t *outputs[16]) {
  switch (type) {
  case P2PKH:
  case BECH32:
#if HASH160_AVX512_AVAILABLE
