...
1. **`scripts/puzzle_scan.py`** - Fetches blockchain data and analyzes puzzle funding
2. **`scripts/wallet_search.py`** - Analyzes solved puzzle keys and generates search commands
3. **`scripts/bip32_search.py`** - BIP-32 derivation path testing and pattern analysis

### Quick Start

```bash
# Run blockchain scan (scripts chdir to repo root)
python scripts/puzzle_scan.py

# Analyze key patterns
python scripts/wallet_search.py

# Generate and run batch search
python scripts/bip32_search.py
