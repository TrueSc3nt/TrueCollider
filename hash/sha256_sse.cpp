...
void sha256sse_checksum(uint32_t *i0, uint32_t *i1, uint32_t *i2, uint32_t *i3,
  ...
  *((uint32_t *)d3) = _byteswap_ulong(s[0].m128i_u32[0]);
  ...
  {
    uint32_t tmp[4];
    _mm_storeu_si128((__m128i*)tmp, s[0]);
    *((uint32_t *)d0) = __builtin_bswap32(tmp[3]);
    *((uint32_t *)d1) = __builtin_bswap32(tmp[2]);
    *((uint32_t *)d2) = __builtin_bswap32(tmp[1]);
    *((uint32_t *)d3) = __builtin_bswap32(tmp[0]);
  }
#endif
