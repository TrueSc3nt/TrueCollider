...
if not exist keyhunt.exe (
  echo [E] keyhunt.exe not found. Build first: bats\00_build\build_mingw_native.bat
  pause
  exit /b 1
)
for %%F in (keyhunt.exe) do set "SZ=%%~zF"
if "!SZ!"=="0" (
  echo [E] keyhunt.exe is 0 bytes ^(corrupt^). Rebuild with bats\00_build\build_mingw_native.bat
  pause
  exit /b 1
)
