...
@(
  "# Deep audit results",
  "",
  "Generated by ``scripts/run_deep_audit.ps1``. Known-hit / dry-run matrix across modes, ``-x`` walks, ``-B`` BSGS helpers, research smokes, and CUDA when ``keyhunt_cuda.exe`` is present.",
  "",
  "``````"
) + $results + @(
  "``````",
